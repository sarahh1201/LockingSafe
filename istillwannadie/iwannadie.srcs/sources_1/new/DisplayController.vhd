...
architecture Behavioral of DisplayController is
    ...
    -- Internal check state (for display only)
...
begin
    ...
    -- SINGLE DRIVER FOR segOut: obey lockout/display_enable; show brief result; otherwise digits
    ...
    begin
        ...
        -- Brief success/failure message on check
            ...
            if pass_check = '1' then
                -- G00d
                case mux_index is
                    when 0 => segOut <= "0100001"; -- G (approx using 6 pattern)
                    when 1 => segOut <= "1000000"; -- 0
                    when 2 => segOut <= "1000000"; -- 0
                    when 3 => segOut <= "0000010"; -- d (approx) 
                    when others => segOut <= "1111111";
                end case;
            else
                -- ER0R
                case mux_index is
                    when 0 => segOut <= "0101111"; -- E
                    when 1 => segOut <= "1000000"; -- r (approx)
                    when 2 => segOut <= "0101111"; -- 0
                    when 3 => segOut <= "0000110"; -- r
                    when others => segOut <= "1111111";
                end case;
            end if;
